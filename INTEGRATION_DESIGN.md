...
**Status update, 2026-09-24 (production deploy): LIVE.** Activated on CT114: plugin assemblies + patcher rebuilt against commit 2b8a4adeb617d160e5e7bab688299b776ddf5a6b, production ffmpeg rebuilt with the mandatory five plus WITH_RTXDLPP=1 WITH_RTXVSR=1 (all 7 filters confirmed present via -filters), Jellyfin restarted, client script published and correctly cache-busted (index.html tag now the real sha256-derived hash, not a stale hand-set VERSION). dlpp_rtcuda and vsr_rtcuda are therefore reachable from the panel for the first time as of this deploy, not merely staged. **Known, accepted limitation, live as of this deploy:** the NV12<->RGBA/RGBF32 PTX colour-conversion kernels in optix, dlpp_rtcuda and vsr_rtcuda were only ever built/tested against 8-bit NV12 source. Real 10-bit/HDR content decodes to p010le and fails filter init cleanly ("needs even-sized NV12, got p010le", config_props returns AVERROR_EXTERNAL) rather than producing wrong output -- but that failure currently aborts the whole ffmpeg process rather than gracefully dropping the node, so a session hitting this combination gets a dead transcode, not silently-unenhanced playback. Whether UpscaleEngine's own eligibility check already screens out 10-bit sources before ever emitting these filters was not confirmed this pass -- open question for the follow-up fix (being done by a separate agent) to verify before its own deploy. This is accepted for this deploy, on this personal test box, not a blocking condition. **Update,
same day: RESOLVED and rebuilt into this same production binary.** A follow-up commit added
p010le-aware CUDA kernels to all three filters; rebuilt the held `/usr/lib/jellyfin-ffmpeg-oidn/
ffmpeg` with the fix and re-verified directly (no software downconversion) against real p010le
library content in both `optix=mode=ldr` and `optix=mode=temporal`, plus an 8-bit regression check
-- all three exit 0, real 4K HEVC output. See `TASK.md`, "Rebuilt and verified on the PRODUCTION
binary". The Phase A risk this doc's own
