/*
 * THE CONTROL DATA. A NEW LEVEL IS DATA: adding one means an entry in an options array; adding a
 * whole axis means an entry here (with its probe key, its group and, if it grades, its rungs) plus
 * one line in LIVE_ROWS (live-block.js). Neither needs a line of rendering code -- see panel-dom.js.
 *
 * Pure data, no imports: every other module reads this one, nothing here reads them.
 */
export var CONTROLS = [
    {
...
export var SR_ALIASES = { light: 'fsrcnnx', standard: 'fsrcnnx', max: 'fsrcnnx-max' };

/* The axes that only act while a game upscaler is running. Named once, used by addParams. */
export var GAME_OPTION_KEYS = ['jitter', 'depth', 'reactive'];
