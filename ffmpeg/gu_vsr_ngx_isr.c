/*
 * gu_vsr_ngx_isr.c: brought over from rtx-video-re (loader/ngx_isr.c), renamed on the way in.
 * Unused by vf_vsr_rtcuda.c (this filter's own path never calls the ngx_isr feature), but
 * gu_vsr_pe_map.c includes it unconditionally, so it has to build clean alongside the rest.
 * Own copy, independent of gu_dlpp_ngx_isr.c. Our own reverse-engineered shim code, no NVIDIA
 * material. See RTXVSR.md.
 *
