/*
 * gu_dlpp_aivp_loader.c: promoted from the `rtx-video-re` spike (`loader/aivp.c`), renamed on
 * the way in, no behaviour changed. Despite the filename this is the generic PPE-family host
 * (AIVP and DLPP share the same export-table shape, see run_dlpp() below); vf_dlpp_rtcuda.c
 * uses only the DLPP path (run_dlpp, its own IID). Our own reverse-engineered shim code, no
 * NVIDIA material. See RTXDLPP.md.
 *
