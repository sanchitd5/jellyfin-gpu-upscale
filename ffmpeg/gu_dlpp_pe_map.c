/*
 * gu_dlpp_pe_map.c: promoted from the personal `rtx-video-re` reverse-engineering spike
 * (`loader/pe_map.c`) so `vf_dlpp_rtcuda.c` builds standalone from this repo, the same way
 * `vf_dlss.c` etc. do. Renamed on the way in (was `pe_map.c`); no behaviour changed. Our own
 * code, no NVIDIA material: it is a from-scratch minimal PE32+ loader and Win64-ABI shim, not
 * derived from or containing any NVIDIA source, header or binary. See RTXDLPP.md.
 *
...
#include "gu_dlpp_ngx_isr.c"
#include "gu_dlpp_aivp_loader.c"
