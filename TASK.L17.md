...
- Best single-field fix: `+0x3a8` (default `0x40`) set to `1` → 29.763/32.424 dB (RGB/Y) vs base
  29.141/31.202, bicubic 34.699/32.929 — closes about 0.6/1.2 dB of a ~5 dB gap.
- Best two-field fix (agent 10): `+0x3a8=1` + `+0x44=1` → 29.809/32.359 — no further improvement
  from a greedy search or the pairwise grid over the top 5 fields.
...
- 114 of 1956 swept dword writes (agent 9) produced no output at all, spanning 26 fields across
  ...
  respectively on separate re-runs — inconsistent with a fixed per-field cause, so this looks
  state-dependent (write order, or memory left over between runs) rather than a bad field per se.
...
35.341/34.705. Bilinear/bypass were not scored under this method on all frames — worth
...
1. **Explain the 114 no-output runs.** Only two of 26 affected fields were spot-checked, and those
   two disagreed with each other on re-run. Get a clean rc/error per field × value (not reused
   process state between writes), and check whether it's write-order-dependent — e.g. does the
   same write fail only after certain other fields were touched in the same process?
2. **A second bias/offset term outside the single argbuf dword space.** Agent 8's finding was that
   ...
   dump postProcess's argbuf and check for a field that looks like an offset/bias-removal term.
...
5. **Multi-field combinations beyond pairs.** Only single fields (agent 9) and the top-5 pairwise
   grid plus a 6-step greedy search (agent 10) were tried. A 3- or 4-field combination search
   was never run — low expected value given the greedy search plateaued, but not eliminated.
